  ...
  class ModelGenerator < ::Rails::Generators::Base
    ...
    def df_model_template
      ...
      unless (starting = models.find { |m| m.name == model_name })
        ...
        starting = gets_list(list: models,
          on_select: Proc.new do |item|
            this_model_info = model_info[item[:focused]]
            item[:focused].name + " (#{(this_model_info[:column_names] + this_model_info[:f_keys].map(&:name).map(&:upcase)).join(', ')})"
          end
        )
      end
      print "\nThinking..."
      ...
      unless (num_hops = ARGV[2]&.to_i)
        ...
        num_hops = gets_list(list: hops_list,
          on_select: Proc.new do |value|
            associations = Hash.new { |h, k| h[k] = 0 }
            index = value[:focused].split(' ').first.to_i - 1
