...
task :clean do
  ...
  when 'mysql'
    %w[test foo bar].each do |db|
      system("mysqladmin drop -f duty_free_#{db} > /dev/null 2>&1")
    end
  when 'postgres'
    %w[test foo bar].each do |db|
      system("dropdb --if-exists duty_free_#{db} > /dev/null 2>&1")
    end
  when nil, 'sqlite'
...
task prepare: [:clean] do
  ...
  when 'mysql'
    %w[test foo bar].each do |db|
      system("mysqladmin create duty_free_#{db}")
      # Migration happens later in spec_helper.
    end
  when 'postgres'
    %w[test foo bar].each do |db|
      system("createdb duty_free_#{db}")
      # Migration happens later in spec_helper.
    end
  when nil, 'sqlite'
