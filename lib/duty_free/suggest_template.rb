    ...
    def self._suggest_template(hops, do_has_many, this_klass, poison_links = [], path = '')
      ...
      this_klass.reflect_on_all_associations.each do |assoc|
        ...
        if is_belongs_to && assoc.options[:polymorphic] # Polymorphic belongs_to?
          ...
          next if (assoc_klass <= PaperTrail::Version && assoc.options[:as]) ||
                ...
                if inverse_foreign_keys.length > 1
                  ...
                  if (inverses = _find_assocs(:belongs_to, assoc_klass, root_class, assoc.foreign_key, errored_assocs)).empty?
                    if inverse_foreign_keys.first.nil?
                      # So we can rule them out, find the belongs_tos that already are inverses of any relevant has_many
                      ...
                      # Remaining belongs_tos are good candidates to eventually become an inverse_of, and we'll now
                      # simply suggest that they establish this as being a foreign_key
    ...
    def self._find_assocs(macro, klass, to_klass, using_fk = nil, errored_assocs)
      ...
      when :has_many, :has_one
        klass.reflect_on_all_associations.each_with_object([]) do |hm_assoc, s|
          next if hm_assoc.macro != macro || errored_assocs.include?(hm_assoc) ||
                  (hm_assoc.klass <= PaperTrail::Version && hm_assoc.options[:as]) # Skip any PaperTrail associations
