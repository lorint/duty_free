  ...
  module Extensions
    ...
    module ClassMethods
      ...
      def df_export(is_with_data = true, import_template = nil)
        ...
        rows = [friendly_columns(import_template)]
        if is_with_data
          # Automatically create a JOINs strategy and select list to get back all related rows
          template_cols, template_joins = recurse_def(import_template[:all], import_template)
          ...
          relation.select(template_cols.map { |x| x.to_s(mapping) }).each do |result|
            rows << template_columns(import_template).map do |col|
        ...
        ActiveRecord::Base.transaction do
          ...
          data.each_with_index do |row, row_num|
            ...
            if is_first # Anticipate that first row has column names
              ...
              template_column_objects = recurse_def(import_template[:all], import_template).first
              ...
              valid_unique = valid_uniques(uniques, cols, starred, import_template)
              ...
              keepers.each do |key, v|
                ...
                if this_path && v.path != this_path.split(',').map(&:to_sym) && !is_has_one
                  if self.respond_to?(:around_import_save)
                    # Send them the sub_obj even if it might be invalid so they can choose to make it
                    # valid if they wish
                    self.around_import_save(sub_obj) do |modded_obj = nil|
                      modded_obj = (modded_obj || sub_obj)
                      modded_obj.save if sub_obj&.valid?
                    end
                  else
                    sub_obj.save if sub_obj&.valid?
                  end
                end
                sub_obj = obj
                this_path = ''
                  ...
                  unless (sub_next = sub_objects[this_path])
                    ...
                    if (sub_next = sub_obj.send(path_part)).nil?
                      ...
                      sub_next = if is_has_one
                                   ...
                                   begin
                                     # Goofs up if trim_prefix isn't the same name as the class, or if it's a self-join?
                                     # (like when trim_prefix == 'Reports To')
                                     # Need to test this more when the self-join is more than one hop away, such as importing orders and having employees come along :)
                                     if sub_obj.class == klass
                                       trim_prefix = ''
                                       # binding.pry
                                     end
                                     sub_unique = klass.valid_uniques(uniques, cols, starred, import_columns, all, trim_prefix)
                                   ...
                                   sub_bt = klass.find_by(criteria) if criteria
                    ...
                    if sub_next.is_a?(ActiveRecord::Associations::CollectionProxy)
                      ...
                      # (Case in point, importing Order with related Order Detail and Product, and then Product needs to be found or built first before OrderDetail.)
                      # Might have to do a deferred save kind of thing, and also make sure the product stuff came first before the other stuff
                      sub_unique = sub_next.klass.valid_uniques(uniques, cols, starred, import_template, all, trim_prefix)
                      # Find by all corresponding columns
      ...
      def friendly_columns(import_template = self::IMPORT_TEMPLATE)
        requireds = (import_template[:required] || [])
        template_columns(import_template).map do |col|
      ...
      def template_columns(import_template = nil)
        if @template_import_template != import_template
          @template_import_template = import_template
          @template_detail_columns = nil
        end
        @template_detail_columns ||= recurse_def(import_template[:all], import_template).first.map(&:to_sym)
      end
      ...
      def valid_uniques(uniques, cols, starred, import_template, all = nil, trim_prefix = '')
        ...
        unless (vus = @valid_uniques[col_list])
          ...
          template_column_objects = recurse_def(all || import_template[:all], import_template).first
    ...
    private
      ...
      def recurse_def(array, import_template, assocs = [], joins = [], pre_prefix = '', prefix = '')
        ...
        array = array.inject([]) do |s, col|
          s += if col.is_a?(Hash)
                 col.inject([]) do |s2, v|
                   ...
                   s2 += recurse_def((v.last.is_a?(Array) ? v.last : [v.last]), import_template, assocs, joins_array, prefixes, v.first.to_sym).first
                 end
               elsif col.nil?
                 if assocs.empty?
                   []
                   ...
                   recurse_def(assocs.last.klass::IMPORT_TEMPLATE[:all], import_template, assocs, joins_array, prefixes).first
                 end
               else
                 [::DutyFree::Column.new(col, pre_prefix, prefix, assocs, self, import_template[:as])]
               end
          s
        end
        [array, joins]
